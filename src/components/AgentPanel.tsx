...
import { Search, CheckCircle2, Loader2, Brain } from "lucide-react";

export interface AgentStep {
  type: "step" | "thinking" | "tool_call" | "tool_result";
  step?: number;
  text?: string;
  name?: string;
...
export function AgentPanel({
  ...
  return (
    <div className="animate-fade-up mx-auto max-w-3xl space-y-4">
      ...
      {/* Current agent panel */}
      <div className="rounded-2xl bg-card card-shadow overflow-hidden border-t-2 border-amber-400">
        {/* Header */}
                ...
                >
                  {s.type === "step" && (
                    <>
                      ...
                      <span className="text-[12px] text-text-tertiary pt-0.5">
                        Analizando…
                      </span>
                    </>
                  )}
                  ...
                  {s.type === "thinking" && (
                    <>
                      <Brain className="mt-0.5 h-4 w-4 text-amber-500 shrink-0" />
                      ...
                      <div className="text-[12px]">
                        ...
                        {s.args?.query ? (
                          <span className="ml-1.5 text-text-tertiary">
                            &ldquo;{String(s.args.query).slice(0, 50)}&rdquo;
                          </span>
                        ) : null}
                      </div>
                    </>
