...
async function main() {
  ...
  if (!apiKey) {
    console.error("Set PINECONE_API_KEY");
    process.exit(1);
  }
  ...
  const indexName = process.env.PINECONE_INDEX_OPENAI || "sherlock-openai";
  ...
  console.log("Done! Pinecone OpenAI ingestion complete.");
